...
├── netlify.toml              # Netlify 部署配置（build / publish / SPA 回退）
├── components/
...
项目已通过 [Netlify](https://www.netlify.com/) 部署，构建配置见仓库根目录的 `netlify.toml`：

- **构建命令**：`npm run build`
- **发布目录**：`dist`
- **SPA 路由**：`/*` 自动回退至 `index.html`

关联 GitHub 仓库（当前为 `ChenYiming-aaa/sku-foundry`）后，每次 `git push` 会自动触发重新部署。
